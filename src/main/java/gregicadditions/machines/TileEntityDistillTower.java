...
public class TileEntityDistillTower extends RecipeMapMultiblockController {
	...
	protected BlockPattern createStructurePattern() {
		return SingleLimitFactory.start(RIGHT, FRONT, UP).aisle("YSY", "YZY", "YYY").aisle("XXX", "X#X", "XXX").setRepeatable(0, 10).aisle("XXX", "XXX", "XXX").where('S', selfPredicate()).where('Z', abilityPartPredicate(MultiblockAbility.IMPORT_FLUIDS)).where('Y', statePredicate(getCasingState()).or(abilityPartPredicate(MultiblockAbility.EXPORT_ITEMS, MultiblockAbility.INPUT_ENERGY))).where('X', statePredicate(getCasingState()).or(abilityPartPredicate(MultiblockAbility.EXPORT_FLUIDS))).where('#', isAirPredicate()).addCheck(1, abilityPartPredicate(MultiblockAbility.EXPORT_FLUIDS)).addCheck(2, abilityPartPredicate(MultiblockAbility.EXPORT_FLUIDS)).build();
	}
