...
public class TileEntityCokeOven extends MultiblockControllerBase {
	...
	private boolean tryPickNewRecipe() {
		...
		if (MetaTileEntity.addItemsToItemHandler(exportItems, true, outputs) && MetaTileEntity.addFluidsToFluidHandler(exportFluids, true, fluidOutputs)) {
	...
	protected BlockPattern createStructurePattern() {
		return FactoryBlockPattern.start().aisle("XXX", "XXX", "XXX").aisle("XXX", "X#X", "XXX").aisle("XXX", "XYX", "XXX").setAmountAtLeast('X', 20).where('X', statePredicate(getCasingState()).or(tilePredicate((state, tile) -> {
		...
		}))).where('#', isAirPredicate()).where('Y', selfPredicate()).build();
	}
	...
	protected ModularUI createUI(EntityPlayer entityPlayer) {
		return ModularUI.builder(GuiTextures.BRONZE_BACKGROUND, 176, 166)
				...
				.widget(new SlotWidget(importItems, 0, 33, 24, true, true).setBackgroundTexture(GuiTextures.BRONZE_SLOT, GuiTextures.BRONZE_FURNACE_OVERLAY)).progressBar(this::getProgressScaled, 58, 24, 20, 15, GuiTextures.BRONZE_BLAST_FURNACE_PROGRESS_BAR, ProgressWidget.MoveType.HORIZONTAL).widget(new SlotWidget(exportItems, 0, 85, 24, true, false).setBackgroundTexture(GuiTextures.BRONZE_SLOT, ClientHandler.COAL_OVERLAY)).widget(new TankWidget(exportFluids.getTankAt(0), 102, 23, 18, 18).setBackgroundTexture(ClientHandler.BRONZE_FLUID_SLOT).setAlwaysShowFull(true).setContainerIO(true, false)).bindPlayerInventory(entityPlayer.inventory, GuiTextures.BRONZE_SLOT).build(getHolder(), entityPlayer);
	}
